---
name: scan-sources
description: (r2p) Read research/wiki/raw/registry.yaml and re-scrape any entry due for re-fetch (now > last_scraped + freq), delegating to the web-scraping skill, deduping by content hash against research/wiki/raw/seen.jsonl, and landing new content in research/wiki/raw/scraped/<slug>/. Use when the user says "scan sources", "/scan-sources", "/scan-sources --slug=<slug>", "/scan-sources --category=<cat>", "/scan-sources --force", or otherwise asks to refresh the project's tracked sources. Always explicit; never auto-fires on a clock.
...
`research/wiki/raw/registry.yaml`. Reads the registry, decides which entries are
due, hands fetching off to the `web-scraping` skill, dedupes by content
sha256, lands fresh content in `research/wiki/raw/scraped/<slug>/`, and updates
`last_scraped` in the registry. Audit trail is git history of the
registry + the per-fetch frontmatter on each `research/wiki/raw/scraped/*` file —
no separate run log.

## Preconditions

- `research/wiki/raw/registry.yaml` exists in the project root. If absent, stop
  and tell the user to seed it from `templates/wiki/raw/registry.yaml`.
- `research/wiki/raw/seen.jsonl` exists (seeded empty by `r2p init`). If absent,
...
- Read `research/wiki/raw/registry.yaml`. Parse with a YAML parser
...
- Read `research/wiki/raw/seen.jsonl`. If a row already exists with the same
  `slug` and `content_sha256`, the content is a duplicate:
  - Skip writing a new file under `research/wiki/raw/scraped/<slug>/`.
...
2. Slugify the title for the filename: lowercase, kebab-case, strip
   ...
   `research/wiki/raw/scraped/<slug>/YYYY-MM-DD_<title-slug>.md`.
...
4. Append one row to `research/wiki/raw/seen.jsonl`:
   ```json
   {"slug": "<slug>", "url": "<url>", "content_sha256": "<hash>", "scraped_at": "<ts>", "raw_path": "research/wiki/raw/scraped/<slug>/<filename>.md"}
   ```

### 6. Update registry `last_scraped`

After each entry (whether fresh, duplicate, or failed), update the
entry's `last_scraped` field in `research/wiki/raw/registry.yaml` to the current
...
Files written: 2
  + research/wiki/raw/scraped/investment-news-cambodia/2026-05-05_new-port-deal.md
  + research/wiki/raw/scraped/portinfra-tracker/2026-05-05_sihanoukville-throughput-q1.md
...
- **Don't auto-ingest into `research/wiki/`.** Scraped content lands in
  `research/wiki/raw/scraped/`. Promotion to `research/wiki/` requires the user to invoke
...
1. Load `research/wiki/raw/registry.yaml`; filter to `category: investment`.
...
4. For each: compute content sha256, check `research/wiki/raw/seen.jsonl`, write
...
- **Does not ingest into `research/wiki/`.** That's `/wiki-ingest`, explicit.
...
- **Does not fetch sources outside the registry.** A URL the
  ...
  this skill. Edits to `research/wiki/raw/registry.yaml` are manual.
