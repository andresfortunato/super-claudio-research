...
```
wiki/raw/   → immutable source material (papers, scrapes, dataset notes)
wiki/       → distilled pages with claims, citations, cross-links
```

A claim in `wiki/` always cites a source page. A source page always cites
a file in `wiki/raw/`. The chain back to evidence is unbroken.

## What goes where

| Goes in `wiki/raw/`                       | Goes in `wiki/<page-type>/`                  |
|-------------------------------------------|----------------------------------------------|
...
If you're tempted to write something in `wiki/raw/`, you're using it wrong.
...
- `/wiki-ingest <wiki/raw/path>` — distill one raw file into wiki pages.
- `/wiki-lint` — audit for orphans, contradictions, stale synthesis, budget violations.
- `/scan-sources` — refresh tracked sources from `wiki/raw/registry.yaml`.
