...
This file is the authoritative spec for `research/wiki/` in this project. Read it before
...
`research/wiki/raw/` is the immutable archive — papers, scrapes, dataset notes. Never edited.

`research/wiki/{sources,concepts,entities,synthesis}/` is the distilled, queryable
knowledge layer built *from* `research/wiki/raw/`. Every claim cites a source page,
which cites a `research/wiki/raw/` file. This is the Karpathy three-layer pattern:
...
|-----------|------------------------------------------------------|-----------|---------------------------------------|
| source    | One ingested item from `raw/` — descriptor, not full content | 300       | `research/wiki/sources/<slug>.md`              |
| concept   | An idea, mechanism, or claim about the world         | 800       | `research/wiki/concepts/<kebab-case-claim>.md` |
| entity    | A named thing: org, place, dataset, person, program  | 600       | `research/wiki/entities/<kebab-case-name>.md`  |
| synthesis | Aggregation across ≥3 sources on a single question   | (none)    | `research/wiki/synthesis/<question-slug>.md`   |

### When to create vs extend

- **Source page**: always one new page per ingested `raw/` file. Never extend.
- **Concept page**: extend if a relevant page exists; create only if the claim is genuinely new. Search `research/wiki/concepts/` first.
...
---
type: source
raw_path: research/wiki/raw/2026-04-12_studwell_how-asia-works.md
...
type: concept
sources:
  - research/wiki/sources/studwell-2013-how-asia-works.md
  - research/wiki/sources/cherif-hasanov-2019-tigers.md
related:
  - research/wiki/concepts/export-discipline-as-industrial-policy-test.md
---
```
...
sources:
  - research/wiki/sources/world-bank-2023-vietnam-cem.md
---
```
...
sources:
  - research/wiki/sources/studwell-2013-how-asia-works.md
  - research/wiki/sources/cherif-hasanov-2019-tigers.md
  - research/wiki/sources/lin-2012-quest.md
---
```
...
- **Slugs**: lowercase kebab-case. Strip articles ("the", "a"). No dates in slugs except for source pages, where the date prefix from `research/wiki/raw/` is encoded in the source page's `slug` field, not the filename.
...
- All links are **relative paths** within `research/wiki/`: `[Studwell 2013](../sources/studwell-2013-how-asia-works.md)`.
- Every concept and entity page MUST link to at least one source page.
- Every source page MUST link back to its `research/wiki/raw/` file (relative path, e.g. `../raw/2026-04-12_studwell_how-asia-works.md`).
...
A researcher drops `research/wiki/raw/2026-04-12_studwell_how-asia-works.md` (a markdown
extract of the book) into `research/wiki/raw/`. They run `/wiki-ingest research/wiki/raw/2026-04-12_studwell_how-asia-works.md`.

The skill:
1. Reads the file. Identifies it as a book about East Asian industrial policy.
2. Searches `research/wiki/concepts/` for existing related pages. Finds none.
3. Creates `research/wiki/sources/studwell-2013-how-asia-works.md` (source, ~250 words).
4. Creates `research/wiki/concepts/agriculture-first-development-sequencing.md` (concept).
5. Creates `research/wiki/concepts/export-discipline-as-industrial-policy-test.md` (concept).
6. Creates `research/wiki/entities/japan-meti.md`, `research/wiki/entities/south-korea-epb.md`.
7. Adds 5 entries to `research/wiki/index.md`.
8. Appends one line to `research/wiki/log.md`.
9. Reports back to the researcher with the list of pages and any caveats.

## What the wiki is NOT

- **Not a dump of `research/wiki/raw/` content.** Source pages are descriptors, not transcripts.
- **Not a personal notebook.** No first-person voice, no daily log entries, no scratch.
- **Not auto-maintained.** `/wiki-ingest` is explicit. Nothing writes to `research/wiki/` automatically.
